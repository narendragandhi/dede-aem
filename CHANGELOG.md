...
- **API key authentication** - every REST/GraphQL endpoint except `/actuator/health` now requires a key via `X-Dede-Api-Key` or `Authorization: Bearer <key>`. Fails closed always: if `DEDE_API_KEY` isn't set, a random key is generated at startup and logged once (Jenkins'/Home Assistant's first-run pattern) rather than leaving the API open. Closes the gap `POST /api/graph/scan`'s path restriction explicitly did not (see ADR 007's note, now superseded). See [ADR 009](docs/adr/0009-static-api-key-authentication.md).
...
- **`docs/PERFORMANCE.md`** - measured scan performance across three real AEM codebases up to 1,433 files (sub-linear time growth, flat ~260-300MB memory), plus a reproducible benchmark (`VulnerabilityServiceBenchmark`, run via `-Dtest=VulnerabilityServiceBenchmark`) that found and confirmed a fix for a super-linear scaling bug in the `--security` reachability audit (see Fixed below).
- **`POST /api/graph/scan`** - Populates the graph in server mode via the API instead of only via CLI args. Fails closed by default (`DEDE_SCAN_ALLOWED_ROOT` required) as a second, independent layer on top of the API key above; without a path restriction any caller holding a valid key could direct the server to walk an arbitrary filesystem path -- confirmed directly against a running server before adding the check. See [ADR 007](docs/adr/0007-scan-endpoint-fails-closed-on-path.md).
...
- **Security reachability audit scaled super-linearly** - `audit()` called `dijkstra.getPath(vuln, endpoint)` once per (vulnerability, endpoint) pair, a full Dijkstra run every time. Measured at 20x more pairs taking 74x longer (0.93s -> 68.9s at 10K -> 200K checks), with per-call cost growing rather than staying flat. Fixed by calling `dijkstra.getPaths(vuln)` once per vulnerability and querying the resulting shortest-path tree per endpoint instead of re-running Dijkstra for every pair -- 23.6x faster at the pessimistic benchmark scale (68.9s -> 2.9s), with per-call cost now flat regardless of scale. All pre-existing correctness tests pass unchanged. See [ADR 008](docs/adr/0008-reachability-audit-single-source-shortest-path-caching.md) and `docs/PERFORMANCE.md`.
